...
function TableData({ marathon, setMarathons, idx, handelDelete, applies = false }) {
    ...
    return (
        <tr className="border-l-base-content border-l-2 text-center">
            <td className="border border-r-base-content border-r-2">{idx + 1}</td>
            <td className="border border-r-base-content border-r-2">{title}</td>
            {
                applies ?
                    <>
                        <td className="border border-r-base-content border-r-2">{new Date(marathonStartDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                    </> :
                    <>
                        <td className="border border-r-base-content border-r-2">{location}</td>
                        <td className="border border-r-base-content border-r-2 hidden lg:table-cell">{runningDistance}</td>
                    </>
            }
            <td className="space-x-2 space-y-2 text-center border border-r-base-content border-r-2">
